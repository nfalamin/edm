<footer class="py-8 px-4 md:px-6 bg-transparent relative z-10">
    <div class="w-full max-w-[96%] 2xl:max-w-[1820px] mx-auto glass-panel p-6 md:p-8 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-6 border border-white/10 dark:border-white/5 shadow-xl shadow-navy-950/10">
        <div class="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 text-center md:text-left">
            ...
            <div class="flex flex-col text-left">
                ...
                <div class="flex items-center gap-3 text-[11px] text-slate-500 mt-1">
                    <a href="<?php echo esc_url(home_url('/privacy/')); ?>" class="hover:text-cyan transition-colors">Privacy Policy</a>
                    <span>•</span>
                    <a href="<?php echo esc_url(home_url('/terms/')); ?>" class="hover:text-cyan transition-colors">Terms &amp; EULA</a>
                    <span>•</span>
                    <a href="<?php echo esc_url(home_url('/edm/')); ?>" class="text-cyan font-bold hover:text-white transition-colors">EDM Hub</a>
                </div>
            </div>
        </div>
...
<div x-data="aiChatbot()" class="fixed bottom-6 right-6 z-[100] flex flex-col items-end font-sans">
    ...
    <button @click="toggle()" class="w-14 h-14 bg-gradient-to-tr from-blue-600 to-cyan-500 text-white rounded-full flex items-center justify-center text-2xl shadow-xl transition-all duration-300 hover:scale-105">
        <i class="fa-solid fa-message" x-show="!isOpen"></i><i class="fa-solid fa-chevron-down" x-show="isOpen" x-cloak></i>
    </button>
</div>