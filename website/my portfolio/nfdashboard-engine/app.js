...
class EdmApp {
    ...
    render32SocketsGrid() {
        ...
        for (let i = 1; i <= 32; i++) {
            ...
            html += `
                <div style="background: #0B0F14; border: 1px solid #26292D; border-radius: 8px; padding: 8px 10px; display: flex; flex-direction: column; gap: 4px;">
    ...
    simulateTestDownload() {
        ...
        if (tbody) {
            ...
            row.innerHTML = `
                ...
                <td>4.2 GB</td>
                <td>
                    <div class="progress-bar-wrap" style="width: 120px; height: 6px; background: rgba(255,255,255,0.1); border-radius: 4px; overflow: hidden; display: inline-block; vertical-align: middle;">
    ...
    handleRemoteDeviceFilter(filter) {
        const deviceCards = document.querySelectorAll(".device-card, .remote-device-row");
        deviceCards.forEach(card => {
            if (!filter || filter === "all") {
                ...
                card.style.display = (status === filter || type === filter) ? "" : "none";
            }
        });
        this.showToast(`Filtered devices by: ${filter}`, "info");
    }
    ...
    handleConfirmGlobalSwitch() {
        ...
        if (btn) {
            btn.className = next === "ON" ? "btn btn-outline-success btn-sm" : "btn btn-outline-danger btn-sm";
        }
    ...
    handleConfirmAsiaSwitch() {
        ...
        if (btn) {
            btn.className = next === "ON" ? "btn btn-outline-primary btn-sm" : "btn btn-outline-secondary btn-sm";
        }
    ...
    markAllNotificationsRead() {
        document.querySelectorAll(".notification-item, .notif-unread").forEach(el => {
            el.classList.remove("unread", "notif-unread");
        });
        const badge = document.getElementById("unread-notification-badge") || document.getElementById("notif-badge");
        if (badge) {
