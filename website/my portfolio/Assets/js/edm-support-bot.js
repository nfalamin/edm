 ...
 * Features 100% Offline Specialized Knowledge Base & EDM Technical Assistant
 * STRICTLY SCOPED TO EXCLUSIVE DOWNLOAD MANAGER (EDM) ONLY
 */
(function() {
    'use strict';
    ...
    const EDM_KNOWLEDGE_BASE = [
        {
            keywords: ['speed', 'turbo', '32', 'socket', 'acceleration', 'fast', 'স্পিড', 'গতি', '৩২', 'কানেকশন', 'থ্রেড'],
        ...
        {
            keywords: ['video', 'youtube', '4k', '8k', 'grabber', 'sniffer', 'ভিডিও', 'ইউটিউব', 'ডাউনলোড', 'মিডিয়া'],
        ...
        {
            keywords: ['extension', 'chrome', 'edge', 'firefox', 'brave', 'opera', 'এক্সটেনশন', 'ব্রাউজার', 'প্লাগইন'],
        ...
        {
            keywords: ['resume', 'crash', 'pause', 'আটকে', 'বন্ধ', 'রেজিউম', 'পজ', 'ব্যর্থ', 'ফেল'],
        ...
        {
            keywords: ['system', 'requirement', 'ram', 'windows', 'os', 'রিকোয়ারমেন্ট', 'উইন্ডোজ', 'র‍্যাম', 'পিসি'],
            response: `<strong>💻 সিস্টেম রিকোয়ারমেন্টস (System Requirements):</strong><br><br>
            • <strong>অপারেটিং সিস্টেম:</strong> Windows 10 (64-bit) বা Windows 11।<br>
            • <strong>প্রসেসর:</strong> Intel / AMD Dual Core বা তদূর্ধ্ব।<br>
            • <strong>র‍্যাম (RAM):</strong> সর্বনিম্ন 2 GB (4 GB রেকমেন্ডেড)।<br>
            • <strong>ফ্রেমওয়ার্ক:</strong> Native .NET 10 Win32 ইঞ্জিন (জিরো ব্লোটওয়্যার)।`
        },
        {
            keywords: ['price', 'pricing', 'subscription', 'license', 'টাকা', 'দাম', 'লাইসেন্স', 'পেমেন্ট', 'বিকাশ', 'নগদ'],
        ...
        {
            keywords: ['torrent', 'magnet', 'টরেন্ট', 'ম্যাগনেট'],
            response: `<strong>🧲 BitTorrent & Magnet Protocol সাপোর্ট:</strong><br><br>
            EDM সরাসরি .torrent ফাইল এবং magnet: লিংক সাপোর্ট করে। বিল্ট-ইন DHT ও পিয়ার-টু-পিয়ার এক্সচেঞ্জের মাধ্যমে কোনো বাড়তি টরেন্ট ক্লায়েন্ট ছাড়াই হাই-স্পিডে টরেন্ট ডাউনলোড সম্পন্ন করে।`
        },
        {
            keywords: ['batch', 'queue', 'scheduler', 'ব্যাচ', 'কিউ', 'শিডিউল'],
            response: `<strong>📋 ব্যাচ ডাউনলোড ও স্মার্ট শিডিউলার:</strong><br><br>
            • টেক্সট ফাইল বা ক্লিপবোর্ড থেকে একসাথে শত শত লিংক ইমপোর্ট করে এক ক্লিকে কিউতে যুক্ত করুন।<br>
            • নির্দিষ্ট সময় পর পর অটো-ডাউনলোড শুরু/বন্ধ এবং ডাউনলোড শেষে পিসি অটো-শাটডাউন করার সুবিধা রয়েছে।`
        },
        {
            keywords: ['obhijog', 'complaint', 'bug', 'idea', 'problem', 'feedback', 'অভিযোগ', 'সমস্যা', 'বাগ', 'পরামর্শ'],
    ...
    // Non-EDM query patterns for polite boundary enforcement
    const NON_EDM_PATTERNS = [
        'weather', 'news', 'politics', 'president', 'cricket', 'football', 'movie', 'song',
        'recipe', 'cook', 'chatgpt', 'openai', 'love', 'who are you', 'how are you',
        'আবহাওয়া', 'রাজনীতি', 'খেলা', 'সিনেমা', 'রান্না', 'প্রেমে', 'গান', 'কেমন আছো'
    ];
    ...
    window.processBotQuery = function(query) {
        ...
        setTimeout(() => {
            ...
            // Check if query is strictly outside EDM scope
            const isNonEdm = NON_EDM_PATTERNS.some(p => qLower.includes(p));

            if (isNonEdm) {
                answer = `আমি শুধুমাত্র <strong>EDM (Exclusive Download Manager)</strong> এর অফিসিয়াল স্পেশালিস্ট টেকনিক্যাল অ্যাসিস্ট্যান্ট।<br><br>
                EDM সফটওয়্যার ডাউনলোড, ৩২-সকেট টার্বো স্পিড, এক্সটেনশন সেটআপ বা কোনো টেকনিক্যাল সমস্যা সম্পর্কিত প্রশ্নের উত্তর দিতে আমি নিয়োজিত। EDM সম্পর্কিত যেকোনো প্রশ্ন জিজ্ঞাসা করতে পারেন।`;
            } else {
                ...
                if (!answer) {
                    answer = `আমি আপনার প্রশ্নটি বুঝতে পেরেছি। EDM সফটওয়্যার সম্পর্কিত আরও বিস্তারিত জানতে বা সরাসরি সমস্যা জানাতে অনুগ্রহ করে আমাদের 
                    ...
                    মেসেজ পাঠান। অথবা <strong>"স্পিড", "ভিডিও", "এক্সটেনশন", "রেজিউম", "সিস্টেম রিকোয়ারমেন্টস"</strong> লিখে জিজ্ঞাসা করুন।`;
                }
            }
