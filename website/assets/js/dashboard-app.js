...
class EdmApp {
    ...
    navigateTo(pageKey) {
        ...
        // Hide all views
        document.querySelectorAll(".view-page, .dash-page-view").forEach(page => page.classList.remove("active"));
    ...
    refreshData() {
        this.renderCurrentView();
        this.showToast("Live data synchronized with Control Plane API", "success");
    }

    // ── 1. RELEASE PUBLISHING & UPLOAD CONTROLLER ──
    handleReleaseFileSelect(input) {
        if (input.files && input.files[0]) {
            const file = input.files[0];
            const sizeMb = (file.size / (1024 * 1024)).toFixed(2);
            this.selectedReleaseFile = file;
            this.showToast(`Selected: ${file.name} (${sizeMb} MB)`, "info");
        }
    }

    publishRelease() {
        const platform = document.getElementById("release-input-platform")?.value || "WindowsDesktop";
        const version = document.getElementById("release-input-ver")?.value?.trim() || "v2.2.0";
        const severity = document.getElementById("release-input-severity")?.value || "Recommended";
        const minVer = document.getElementById("release-input-minver")?.value?.trim() || "v1.9.0";
        const notes = document.getElementById("release-input-notes")?.value?.trim() || "Turbocharged performance update";
        const fileObj = this.selectedReleaseFile;

        const fileName = fileObj ? fileObj.name : `EDM-Setup-${version}.exe`;
        const fileSize = fileObj ? `${(fileObj.size / (1024 * 1024)).toFixed(1)} MB` : "2.4 MB";
        const fakeSha = Array.from({length: 64}, () => Math.floor(Math.random() * 16).toString(16)).join("");

        const newRelease = {
            version: version.startsWith("v") ? version : `v${version}`,
            name: `${platform} Production Build`,
            date: new Date().toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }),
        ...
        // Prepend to dataset
        if (!window.EDM_MOCK_DATA.releases) window.EDM_MOCK_DATA.releases = [];
        window.EDM_MOCK_DATA.releases.unshift(newRelease);

        // Save to Shared Local Storage for /edm
        try {
            localStorage.setItem("edm_live_latest_release", JSON.stringify(newRelease));
            localStorage.setItem("edm_shared_product_releases", JSON.stringify(window.EDM_MOCK_DATA.releases));
        } catch (e) {}
        ...
        try {
            if (typeof BroadcastChannel !== "undefined") {
                ...
                bus.postMessage({ type: "PRODUCT_STATE_CHANGED", payload: newRelease });
            }
        } catch (e) {}

        // Append Audit Log
        if (window.EDM_MOCK_DATA.auditLogs) {
            window.EDM_MOCK_DATA.auditLogs.unshift({
        ...
        this.renderReleasesTable();
        this.showToast(`Release ${version} published live to /edm!`, "success");
    }

    rollbackRelease(version) {
        if (confirm(`Are you sure you want to rollback ${version} to previous stable build?`)) {
            if (window.EDM_MOCK_DATA.auditLogs) {
                window.EDM_MOCK_DATA.auditLogs.unshift({
                    timestamp: new Date().toISOString().replace("T", " ").substring(0, 19),
                    admin: "Super Admin",
            ...
            this.showToast(`Rollback executed for ${version}`, "warning");
            this.renderReleasesTable();
        }
    }

    // ── 2. LANDING CMS HERO & CONTENT CONTROLLER ──
    saveLandingContent() {
        const pill = document.getElementById("cms-input-pill")?.value || "";
        const title = document.getElementById("cms-input-title")?.value || "";
        const subtitle = document.getElementById("cms-input-subtitle")?.value || "";
        const ctaPrimary = document.getElementById("cms-input-cta-primary")?.value || "";
        const snifferPlaceholder = document.getElementById("cms-input-sniffer-placeholder")?.value || "";
        ...
        try {
            localStorage.setItem("edm_landing_content", JSON.stringify(contentData));
        } catch (e) {}

        // Broadcast to /edm tabs
        try {
            if (typeof BroadcastChannel !== "undefined") {
                const bus = new BroadcastChannel("edm_product_state_bus");
                bus.postMessage({ type: "PRODUCT_STATE_CHANGED", payload: contentData });
            }
        } catch (e) {}
        ...
        this.showToast("Landing content updated & propagated to /edm!", "success");
    }

    // ── 3. PROMOTIONS & COUPONS CONTROLLER ──
    savePromotion() {
        const code = document.getElementById("promo-input-code")?.value?.trim().toUpperCase() || "OFFER";
        const discount = document.getElementById("promo-input-discount")?.value?.trim() || "20% OFF";
        const type = document.getElementById("promo-input-type")?.value || "Percentage";
        ...
        if (!window.EDM_MOCK_DATA.promotions) window.EDM_MOCK_DATA.promotions = [];
        window.EDM_MOCK_DATA.promotions.unshift(newPromo);

        try {
            localStorage.setItem("edm_promotions", JSON.stringify(window.EDM_MOCK_DATA.promotions));
        } catch (e) {}
        ...
        try {
            if (typeof BroadcastChannel !== "undefined") {
                const bus = new BroadcastChannel("edm_product_state_bus");
                bus.postMessage({ type: "PRODUCT_STATE_CHANGED", payload: newPromo });
            }
        } catch (e) {}

        this.closeModal("modal-promotion");
        this.renderPromotionsTable();
        this.showToast(`Coupon ${code} activated! Live on /edm hero.`, "success");
    }

    togglePromotionStatus(code) {
        if (!window.EDM_MOCK_DATA.promotions) return;
        const promo = window.EDM_MOCK_DATA.promotions.find(p => p.code === code);
        if (promo) {
            ...
            this.renderPromotionsTable();
            this.showToast(`Coupon ${code} is now ${promo.status}`, "info");
        }
    }

    // ── 4. 30-DAY TRIAL & LICENSING CONFIG CONTROLLER ──
    saveTrialConfig() {
        const duration = document.getElementById("trial-input-duration")?.value || 30;
        const grace = document.getElementById("trial-input-grace")?.value || 3;
        const maxDevices = document.getElementById("trial-input-maxdevices")?.value || 5;
        const offline = document.getElementById("trial-input-offline")?.value || 72;
        const hwidEnforce = document.getElementById("trial-input-hwid-enforce")?.checked ?? true;

        const config = { duration, grace, maxDevices, offline, hwidEnforce, updatedAt: new Date().toISOString() };
        try {
            localStorage.setItem("edm_trial_config", JSON.stringify(config));
        } catch (e) {}

        const kpiDays = document.getElementById("kpi-trial-days");
        if (kpiDays) kpiDays.textContent = `${duration} Days`;

        const kpiDevices = document.getElementById("kpi-max-devices");
        if (kpiDevices) kpiDevices.textContent = `${maxDevices} Devices`;

        const kpiOffline = document.getElementById("kpi-offline-hours");
        if (kpiOffline) kpiOffline.textContent = `${offline} Hours`;

        this.closeModal("modal-trial-config");
        this.showToast("30-Day Trial & Licensing policy updated successfully!", "success");
    }

    // ── 5. USER CRUD CONTROLLER ──
    saveUser() {
        ...
        const plan = document.getElementById("user-input-plan")?.value || "Trial";
        const status = document.getElementById("user-input-status")?.value || "Active";
        const country = document.getElementById("user-input-country")?.value || "United States";
        ...
        const newUser = {
            ...
            plan,
            status,
            trial: plan === "Trial" ? "30 days left" : "Completed",
            devices: 1,
            lastActive: "Just now",
            created: new Date().toISOString().substring(0, 10),
            hwid: `HWID-${Array.from({length: 4}, () => Math.floor(Math.random() * 0xFFFF).toString(16).toUpperCase()).join("-")}`,
            downloadsCount: 0,
            bandwidthUsed: "0 GB"
        };

        if (!window.EDM_MOCK_DATA.users) window.EDM_MOCK_DATA.users = [];
        window.EDM_MOCK_DATA.users.unshift(newUser);
    ...
    exportPerformanceReport() {
        const reportData = {
            ...
            promotions: window.EDM_MOCK_DATA.promotions,
            trialConfig: localStorage.getItem("edm_trial_config") || { durationDays: 30 }
        };
        ...
        this.showToast("Performance and distribution report exported", "success");
    }
}

// Global DOM Hook & Dashboard Instance Aliases
document.addEventListener("DOMContentLoaded", () => {
    window.edmApp = new EdmApp();
    window.edmDashboard = window.edmApp; // Alias for all onclick bindings
    ...
    document.querySelectorAll("#cmd-results-list .cmd-item").forEach(item => {
        item.addEventListener("click", () => {
            ...
            if (action === "nav") {
                ...
                if (target === "create-release") window.edmApp.openModal("modal-release");
