...
class EdmWebsiteEngine {
    ...
    async syncProductState() {
        try {
                ...
                if (res.ok) {
                    ...
                    if (data && data.version) {
                        latest = {
                            ...
                            size: data.fileSizeBytes > 0 ? `${(data.fileSizeBytes / (1024 * 1024)).toFixed(1)} MB` : "2.4 MB",
            ...
            // 2. Hydrate Notice Bar & Hero Badges
            ...
            if (dlPrimaryBtn) {
                dlPrimaryBtn.href = latest.downloadUrl || "/api/v1/releases/latest/download";
            }
    ...
    renderChangelogFeed(releases) {
        ...
        changelogFeed.innerHTML = releases.map((rel, idx) => {
            ...
            const size = art?.fileSizeBytes > 0 ? `${(art.fileSizeBytes / (1024 * 1024)).toFixed(1)} MB` : '2.4 MB';
