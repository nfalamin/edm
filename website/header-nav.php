...
<header id="main-header" class="site-header sticky top-0 z-50 glass-panel border-b border-white/5 transition-all duration-300">
    <div class="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <nav class="hidden lg:flex items-center space-x-7 text-slate-300">
            ...
            <a href="<?php echo esc_url(home_url('/edm/')); ?>" class="nav-link text-xs font-bold tracking-wider uppercase text-cyan hover:text-cyan-light transition-colors flex items-center gap-1.5"><span class="w-1.5 h-1.5 rounded-full bg-cyan animate-pulse"></span> EDM App</a>
            <a href="<?php echo esc_url(home_url('/dashboard/')); ?>" class="nav-link text-xs font-semibold tracking-wider uppercase text-slate-400 hover:text-white transition-colors">Dashboard</a>
        </nav>

        <!-- Theme Toggle & CTA Desktop -->
        <div class="hidden lg:flex items-center space-x-4">
            <button @click="darkMode = !darkMode" class="text-slate-500 hover:text-cyan transition-colors focus:outline-none text-lg" aria-label="Toggle Theme">
        ...
        <div class="lg:hidden flex items-center space-x-4">
            <button @click="darkMode = !darkMode" class="text-slate-500 hover:text-cyan transition-colors focus:outline-none text-lg" aria-label="Toggle Theme"><i class="fa-solid fa-moon" x-show="!darkMode"></i><i class="fa-solid fa-sun" x-show="darkMode" x-cloak></i></button>
            <button @click="mobileMenuOpen = !mobileMenuOpen" class="text-slate-400 hover:text-cyan focus:outline-none" aria-label="Toggle Menu"><i class="fa-solid" :class="mobileMenuOpen ? 'fa-xmark text-xl' : 'fa-bars text-xl'"></i></button>
        </div>
    </div>
    ...
    <div x-show="mobileMenuOpen" x-cloak class="lg:hidden absolute top-20 left-0 w-full glass-panel border-b border-white/10 py-6 px-6 z-40">
        <nav class="flex flex-col space-y-4">
            ...
            <a href="<?php echo esc_url(home_url('/edm/')); ?>" @click="mobileMenuOpen = false" class="nav-link text-sm tracking-wider uppercase text-cyan font-bold py-2 border-b border-white/5 flex items-center justify-between"><span>EDM Product Hub</span> <i class="fa-solid fa-arrow-up-right-from-square text-xs"></i></a>
            <a href="<?php echo esc_url(home_url('/dashboard/')); ?>" @click="mobileMenuOpen = false" class="nav-link text-sm tracking-wider uppercase text-slate-300 py-2 border-b border-white/5 flex items-center justify-between"><span>EDM Dashboard</span> <i class="fa-solid fa-gauge-high text-xs"></i></a>
