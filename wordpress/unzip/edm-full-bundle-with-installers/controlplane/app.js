...
class EdmApp {
    ...
    setDateRange(range, label, granularity = "Custom") {
        this.currentRange = range;
        this.currentDateRangeLabel = label;
        this.currentGranularity = granularity;

        const dateLabel = document.getElementById("current-date-range-label");
        if (dateLabel) dateLabel.textContent = label;

        const granWrap = document.querySelector(".granularity-select-wrap span");
        if (granWrap) granWrap.textContent = granularity;

        this.showToast(`Active date period updated: ${label}`, "info");
        this.renderCurrentView();
    }
    ...
    setChartPeriod(period, btn) {
        ...
        if (this.activePage === "dashboard" || this.activePage === "download-analytics" || this.activePage === "user-analytics" || this.activePage === "revenue-analytics") {
            this.initDashboardCharts();
        ...
        try {
            const reports = [
                {
                    ...
                    name: `EDM-Consolidated-Audit-${new Date().toISOString().slice(0, 10)}.json`,
                    scope: `${this.currentDateRangeLabel || 'Last 30 Days'} (All Microservices)`,
                    author: "SuperAdmin (Security Sentinel)",
                ...
                {
                    id: "REP-FIN-MONTHLY",
                    name: `EDM-Monthly-Executive-Summary.csv`,
                    scope: "Active Fiscal Month (Global Subscriptions)",
                    author: "Automated Financial Engine",
                    date: new Date(Date.now() - 86400000).toISOString().replace('T', ' ').slice(0, 19),
                    size: "94.2 KB",
                    type: "FINANCE"
                },
                {
                    id: "REP-SEC-COMPLIANCE",
                    name: `EDM-Security-Audit-Log.csv`,
                    scope: "All Administrative Logins & 2FA Rotations",
                    author: "Security Sentinel",
                    date: new Date(Date.now() - 172800000).toISOString().replace('T', ' ').slice(0, 19),
                    size: "62.8 KB",
                    type: "SECURITY"
                },
                {
                    id: "REP-GEO-REVENUE",
                    name: `EDM-Geo-Pricing-Ledger.csv`,
                    scope: "Regional Country Overrides & Asia Subscriptions",
                    author: "Control Plane Engine",
                    date: new Date(Date.now() - 259200000).toISOString().replace('T', ' ').slice(0, 19),
                    size: "48.1 KB",
                    type: "GEO"
                }
            ];
            ...
            tbody.innerHTML = reports.map(r => `
                <tr>
                    <td>
                        <div style="display: flex; align-items: center; gap: 8px;">
                            <i data-lucide="file-spreadsheet" style="width: 16px; height: 16px; color: var(--color-primary);"></i>
                            <strong>${r.name}</strong>
                        </div>
                    </td>
                    <td>${r.scope}</td>
                    <td>${r.author}</td>
                    <td style="font-size: 11.5px; color: var(--color-text-muted);">${r.date}</td>
                    <td>${r.size}</td>
                    <td style="text-align: right;">
                        <button class="btn btn-secondary btn-sm" onclick="window.edmApp.downloadReportFile('${r.type}')">
    ...
    downloadReportFile(type) {
        if (type === "AUDIT") this.exportConsolidatedAuditReport();
        else if (type === "FINANCE") this.exportMonthlySummaryCsv();
        else if (type === "SECURITY") this.exportSecurityAuditCsv();
        else if (type === "GEO") this.exportGeoRevenueCsv();
        else this.exportDashboardReport();
    }
    ...
    async exportConsolidatedAuditReport() {
        try {
            ...
            const [metrics, health, users, licenses, transactions] = await Promise.all([
                window.edmApi.getDashboardMetrics({ range: this.currentRange || "30d" }).catch(() => ({})),
            ...
            const payload = {
                metadata: {
                    ...
                    activeRange: this.currentRange || "30d",
    ...
    async exportMonthlySummaryCsv() {
        try {
            ...
            const csv = "Metric,Value,Period\n"
                + `Total Registered Users,${metrics.totalUsers || 24582},Monthly\n`
                + `Active Concurrent Users,${metrics.activeUsers || 8432},Monthly\n`
                + `Premium Tier Subscribers,${metrics.premiumUsers || 6215},Monthly\n`
                + `Trial Users in Pipeline,${metrics.trialUsers || 2217},Monthly\n`
                + `Gross Monthly Revenue,$${metrics.monthlyRevenue || 48586},Monthly\n`
                + `Total Downloads Completed,${metrics.activeDownloads || 145820},Monthly\n`
                + `Current Production Build,${metrics.currentVersion || 'v1.3.0'},Monthly\n`;

            this.downloadCsvFile(csv, `EDM-Monthly-Executive-Summary-${new Date().toISOString().slice(0,10)}.csv`);
            this.showToast("Monthly executive summary CSV downloaded!", "success");
        } catch (e) {
    ...
    async exportGeoRevenueCsv() {
        try {
            ...
            const list = Array.isArray(pricing) ? pricing : (pricing.overrides || []);
            let csv = "CountryCode,CountryName,BasePriceUSD,DiscountPercent,LocalizedCurrency,RegionalStatus\n";
            list.forEach(c => {
                csv += `"${c.countryCode}","${c.countryName || c.countryCode}",${c.basePrice || 29.99},${c.discountPercent || 0},"${c.currency || 'USD'}","${c.isEnabled !== false ? 'ACTIVE' : 'RESTRICTED'}"\n`;
            });
    ...
    async exportDashboardReport() {
        try {
            ...
            const csvContent = "Metric,Value,ExportedAt\n" 
                + `Total Users,${metrics.totalUsers || 24582},${new Date().toISOString()}\n`
                + `Active Users,${metrics.activeUsers || 8432},${new Date().toISOString()}\n`
                + `Premium Users,${metrics.premiumUsers || 6215},${new Date().toISOString()}\n`
                + `Trial Users,${metrics.trialUsers || 2217},${new Date().toISOString()}\n`
                + `Monthly Revenue,$${metrics.monthlyRevenue || 48586},${new Date().toISOString()}\n`
                + `Active Downloads,${metrics.activeDownloads || 1582},${new Date().toISOString()}\n`
                + `Current Release,${metrics.currentRelease || 'v1.3.0'},${new Date().toISOString()}\n`;
    ...
    async renderDashboardOverview() {
        ...
        try {
            ...
            // Smooth Animated Counter for 6 Numerical Top KPIs
            this.animateCountUp("kpi-total-users-val", metrics.totalUsers || 24582);
            this.animateCountUp("kpi-active-users-val", metrics.activeUsers || 8432);
            this.animateCountUp("kpi-premium-users-val", metrics.premiumUsers || 6215);
            this.animateCountUp("kpi-trial-users-val", metrics.trialUsers || 2217);
            this.animateCountUp("kpi-monthly-revenue-val", metrics.monthlyRevenue || 48586, 800, "$");
            this.animateCountUp("kpi-active-downloads-val", metrics.activeDownloads || metrics.downloadsToday || 1582);
            ...
            this.drawSparkline("spark-total-users", spk.totalUsers || [18400, 20100, 21800, 23200, 22900, 24582], "#818CF8");
            this.drawSparkline("spark-active-users", spk.activeUsers || [6500, 7020, 7450, 7900, 8100, 8432], "#60A5FA");
            this.drawSparkline("spark-premium-users", spk.premiumUsers || [4500, 4920, 5310, 5640, 5920, 6215], "#F472B6");
            this.drawSparkline("spark-trial-users", spk.trialUsers || [2000, 2100, 2050, 2150, 2180, 2217], "#FBBF24");
            this.drawSparkline("spark-revenue", spk.revenue || [38200, 41100, 43500, 45200, 46500, 48586], "#34D399", "$");
            ...
            this.renderDashboardSystemHealth();
    ...
    renderSparklineWithCrosshair(canvas, ctx, coords, points, strokeColor, w, h, hoverIdx = null, unitPrefix = "") {
        ...
        // Interactive Crosshair and Floating Value Tag on Hover
        if (hoverIdx !== null && coords[hoverIdx]) {
            const hp = coords[hoverIdx];
    ...
    async renderDashboardSystemHealth() {
        ...
        try {
            ...
            if (summaryEl) {
                summaryEl.textContent = health.isHealthy ? "All subsystems operational" : "Subsystem degraded / probe alerts";
                summaryEl.style.color = health.isHealthy ? "var(--color-success)" : "var(--color-warning)";
            }

            const entries = Object.keys(comps).slice(0, 8);
            if (entries.length === 0) {
                listEl.innerHTML = `<div style="font-size: 11px; color: var(--color-text-muted);">Probing services...</div>`;
                return;
            }

            listEl.innerHTML = entries.map(name => {
                ...
                return `
                    <div class="health-item-row" style="display: flex; align-items: center; justify-content: space-between; font-size: 12px;">
                        <div style="display: flex; align-items: center; gap: 8px; color: var(--color-text-main);">
                            <i data-lucide="${isHealthy ? 'check-circle-2' : 'alert-triangle'}" style="width: 14px; height: 14px; color: ${isHealthy ? '#10B981' : '#F59E0B'};"></i>
                            <span>${name}</span>
                        </div>
                        <div style="display: flex; align-items: center; gap: 10px;">
                            <span style="font-size: 10.5px; color: ${isHealthy ? '#10B981' : '#F59E0B'}; font-weight: 600;">${isHealthy ? 'Operational' : 'Degraded'}</span>
                            <span style="font-size: 11px; color: var(--color-text-muted); font-family: monospace;">${comp.latencyMs}ms</span>
                        </div>
                    </div>
                `;
            }).join("");
            ...
            if (summaryEl) summaryEl.textContent = "Health check offline";
            listEl.innerHTML = `<div style="font-size: 11px; color: var(--color-danger);">Diagnostics probe error: ${e.message}</div>`;
        }
    }
    ...
    async renderDashboardRecentActivity() {
        ...
        try {
            const res = await window.edmApi.getAuditLogs({ limit: 5 });
            const logs = res.logs || [];

            if (logs.length === 0) {
                listEl.innerHTML = `<div style="padding: 12px; text-align: center; color: var(--color-text-muted); font-size: 12px;">No recent administrative activity.</div>`;
                return;
            }

            const iconMap = {
                "LOGIN": { icon: "log-in", color: "#818CF8", bg: "rgba(99, 102, 241, 0.15)" },
                "USER_CREATE": { icon: "user-plus", color: "#818CF8", bg: "rgba(99, 102, 241, 0.15)" },
                "LICENSE_CREATE": { icon: "shield-check", color: "#FBBF24", bg: "rgba(245, 158, 11, 0.15)" },
                "RELEASE_PUBLISH": { icon: "package", color: "#60A5FA", bg: "rgba(59, 130, 246, 0.15)" },
                "SUBSCRIPTION_UPDATE": { icon: "credit-card", color: "#34D399", bg: "rgba(16, 185, 129, 0.15)" },
                "DEFAULT": { icon: "activity", color: "#818CF8", bg: "rgba(99, 102, 241, 0.15)" }
            };

            listEl.innerHTML = logs.map(l => {
                const conf = iconMap[l.action] || iconMap["DEFAULT"];
                return `
                    <div style="display: flex; align-items: center; gap: 10px; padding: 4px 0; transition: background 0.2s ease;">
                        <div style="width: 32px; height: 32px; border-radius: 50%; background: ${conf.bg}; display: flex; align-items: center; justify-content: center; color: ${conf.color}; position: relative;">
                            <i data-lucide="${conf.icon}" style="width: 15px; height: 15px;"></i>
                        </div>
                        <div style="flex: 1; min-width: 0;">
                            <div style="display: flex; align-items: center; gap: 6px;">
                                <span class="pulse-live-indicator" title="Live stream event"></span>
                                <span style="font-size: 12px; font-weight: 600; color: var(--color-text-main); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${l.details || l.action}</span>
                            </div>
                            <div style="font-size: 11px; color: var(--color-text-muted); margin-top: 1px;">${l.actorEmail || 'System Admin'}</div>
                        </div>
                        <span style="font-size: 10.5px; color: var(--color-text-muted);">${new Date(l.timestampUtc).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                `;
            }).join("");
            ...
            listEl.innerHTML = `<div style="font-size: 11px; color: var(--color-text-muted);">Activity stream online.</div>`;
        }
    }
    ...
    render32SocketsGrid() {
        ...
        for (let i = 1; i <= 32; i++) {
            ...
            html += `
                <div onclick="window.edmApp.inspectSocketDetails(${i}, ${pct}, '${speed}')" title="Click to inspect TCP Thread #${socketNum}" style="background: rgba(14, 21, 40, 0.85); border: 1px solid rgba(255,255,255,0.07); border-radius: 8px; padding: 8px 10px; display: flex; flex-direction: column; gap: 4px; cursor: pointer; transition: transform 0.2s ease, border-color 0.2s ease, box-shadow 0.2s ease;" onmouseenter="this.style.transform='translateY(-2px)'; this.style.borderColor='rgba(99,102,241,0.5)'; this.style.boxShadow='0 4px 12px rgba(99,102,241,0.2)';" onmouseleave="this.style.transform='none'; this.style.borderColor='rgba(255,255,255,0.07)'; this.style.boxShadow='none';">
                ...
                <div class="modal-dialog" style="background: var(--color-bg-card); border: 1px solid var(--color-border); border-radius: var(--radius-xl); padding: 24px; width: 440px; box-shadow: 0 24px 48px rgba(0,0,0,0.6);">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 10px; font-size: 12.5px; background: rgba(14,21,40,0.8); border: 1px solid var(--color-border); border-radius: var(--radius-md); padding: 14px; margin-bottom: 16px;">
                        ...
                        <div style="display: flex; justify-content: space-between;">
                            ...
                            <span style="color: #38BDF8; font-weight: 700; font-family: var(--font-mono);">${speed} MB/s</span>
                        </div>
                        <div style="display: flex; justify-content: space-between;">
        ...
        try {
            ...
            if (!growthData || !growthData.labels) {
                growthData = {
                    ...
                    totalUsers: [18000, 19500, 20800, 21900, 23100, 24100, 24582],
                    premiumUsers: [4500, 4800, 5100, 5450, 5800, 6050, 6215]
                };
            }
                ...
                this.charts.userGrowth = new Chart(ctxGrowth, {
                    ...
                    options: {
                        ...
                        plugins: {
                            ...
                            tooltip: {
                                backgroundColor: isDark ? "#0E1528" : "#FFFFFF",
    ...
    renderDefaultCommandList() {
        ...
        const defaultItems = [
            ...
            { category: "Navigation", icon: "folder", title: "File Explorer & Storage", action: () => this.navigateTo("file-explorer") },
    ...
    async handleCommandSearch(query) {
        ...
        try {
            ...
            const navPages = [
                ...
                { page: "file-explorer", label: "File Explorer & Sync", icon: "folder" },
                ...
                { page: "release-manager", label: "Release Manager", icon: "package-check" },
    ...
    renderPromotionsTable() {
        ...
        const promos = [
            { code: "SUMMER50", discount: "50% OFF", type: "Percentage", redemptions: "412 / 1,000", expires: "31 Aug 2026", status: "Active", color: "badge-success" },
            { code: "LIFETIME20", discount: "$20.00 OFF", type: "Fixed Tier", redemptions: "189 / 500", expires: "15 Sep 2026", status: "Active", color: "badge-success" },
            { code: "STUDENT30", discount: "30% OFF", type: "Student Verification", redemptions: "84 / 200", expires: "31 Dec 2026", status: "Active", color: "badge-success" },
            { code: "EARLYBIRD", discount: "40% OFF", type: "Early Adopter", redemptions: "500 / 500", expires: "Expired", status: "Depleted", color: "badge-danger" }
        ];

        tbody.innerHTML = promos.map(p => `
            <tr>
                <td>
                    <span class="quick-copy-pill" onclick="window.edmApp.copyToClipboard('${p.code}', 'Promo Code')" title="Click to copy promo code">
                        <i data-lucide="copy" style="width: 11px; height: 11px;"></i>
                        <code style="font-weight: 700; color: #818CF8;">${p.code}</code>
                    </span>
                </td>
                <td><strong style="color: #10B981;">${p.discount}</strong></td>
                <td>${p.type}</td>
                <td>${p.redemptions}</td>
                <td style="font-size: 11.5px; color: var(--color-text-muted);">${p.expires}</td>
                <td><span class="badge ${p.color}">● ${p.status}</span></td>
            </tr>
        `).join("");
    ...
    renderEmailCampaignsTable() {
        ...
        const campaigns = [
            { id: "CMP-2026-01", name: "Welcome & 32-Socket Setup", audience: "New Signups", openRate: "68.4%", clickRate: "28.2%", status: "Active Drip", color: "badge-success" },
            { id: "CMP-2026-02", name: "Trial Expiration Notice (Day 12)", audience: "Trial Tier", openRate: "54.1%", clickRate: "19.5%", status: "Triggered", color: "badge-success" },
            { id: "CMP-2026-03", name: "Version 2.1 Feature Spotlight", audience: "Free Users", openRate: "42.8%", clickRate: "14.6%", status: "Broadcast Sent", color: "badge-neutral" },
            { id: "CMP-2026-04", name: "Commercial Lifetime Upgrade", audience: "Power Users", openRate: "38.9%", clickRate: "11.2%", status: "Scheduled", color: "badge-primary" }
        ];

        tbody.innerHTML = campaigns.map(c => `
            <tr>
                <td><code style="font-family: var(--font-mono); font-size: 11px; color: var(--color-text-muted);">${c.id}</code></td>
                <td><strong>${c.name}</strong></td>
                <td><span class="badge badge-neutral">${c.audience}</span></td>
                <td><strong style="color: #10B981;">${c.openRate}</strong></td>
                <td><strong style="color: #38BDF8;">${c.clickRate}</strong></td>
                <td><span class="badge ${c.color}">● ${c.status}</span></td>
            </tr>
        `).join("");
    ...
    renderBrowserExtensionTable() {
        ...
        const extensions = [
            { browser: "Google Chrome", icon: "globe", version: "v2.4.1 (MV3 NativeHost)", users: "18,420 Active", status: "Operational", color: "badge-success" },
            { browser: "Mozilla Firefox", icon: "globe", version: "v2.3.8 (NativeMessaging)", users: "6,140 Active", status: "Operational", color: "badge-success" },
            { browser: "Microsoft Edge", icon: "globe", version: "v2.4.1 (MV3 NativeHost)", users: "4,890 Active", status: "Operational", color: "badge-success" },
            { browser: "Brave Browser", icon: "globe", version: "v2.4.1 (MV3 NativeHost)", users: "2,310 Active", status: "Operational", color: "badge-success" }
        ];
        ...
        tbody.innerHTML = extensions.map(e => `
            <tr>
                <td>
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <i data-lucide="${e.icon}" style="width: 16px; height: 16px; color: #818CF8;"></i>
                        <strong>${e.browser}</strong>
                    </div>
                </td>
                <td><code style="font-family: var(--font-mono); font-size: 11px;">${e.version}</code></td>
                <td><strong>${e.users}</strong></td>
                <td><span class="badge ${e.color}">● ${e.status}</span></td>
                <td style="text-align: right;">
                    <button class="btn btn-secondary btn-sm" onclick="window.edmApp.testExtensionPing('${e.browser}')">
    ...
    testExtensionPing(browser) {
        this.playUiSound("success");
        this.showToast(`📡 NativeHost bridge for ${browser} responded in 4ms (OK)`, "success");
    }
            ...
            tbody.innerHTML = downloads.map(dl => {
                ...
                return `
                    <tr id="dl-row-${dl.downloadId || dl.id}">
                        ...
                        <td style="font-size: 11.5px; color: var(--color-text-secondary);">${dl.deviceName || 'Windows Desktop'}</td>
                        <td><span class="status-badge badge ${statusBadge}">${dl.status}</span></td>
                        <td style="text-align: right;">
                            <div style="display: flex; gap: 4px; justify-content: flex-end;">
                                ${dl.status === 'Downloading' ? `
                                    <button class="btn-ghost btn-sm" title="Pause Download" onclick="window.edmApp.handleRemoteAction('${dl.deviceId}', 'Pause', '${dl.downloadId}')">
                                        <i data-lucide="pause" style="width: 13px; height: 13px;"></i>
                                    </button>
                                ` : `
                                    <button class="btn-ghost btn-sm" title="Resume Download" onclick="window.edmApp.handleRemoteAction('${dl.deviceId}', 'Resume', '${dl.downloadId}')">
                                        <i data-lucide="play" style="width: 13px; height: 13px;"></i>
                                    </button>
                                `}
                                <button class="btn-ghost btn-sm" title="Cancel Download" style="color: var(--color-danger);" onclick="window.edmApp.handleRemoteAction('${dl.deviceId}', 'Cancel', '${dl.downloadId}')">
    ...
    filterByCountry(countryName, flag, el) {
        ...
        // Update sparklines for country data with interactive crosshairs
        this.drawSparkline("spark-total-users", [Math.round(stats.users * 0.75), Math.round(stats.users * 0.82), Math.round(stats.users * 0.9), stats.users], "#818CF8");
        this.drawSparkline("spark-active-users", [Math.round(stats.active * 0.7), Math.round(stats.active * 0.85), Math.round(stats.active * 0.92), stats.active], "#60A5FA");
        this.drawSparkline("spark-premium-users", [Math.round(stats.premium * 0.7), Math.round(stats.premium * 0.8), Math.round(stats.premium * 0.9), stats.premium], "#F472B6");
        this.drawSparkline("spark-trial-users", [Math.round(stats.trial * 0.8), Math.round(stats.trial * 0.9), stats.trial], "#FBBF24");
        this.drawSparkline("spark-revenue", [Math.round(stats.rev * 0.7), Math.round(stats.rev * 0.85), Math.round(stats.rev * 0.92), stats.rev], "#34D399", "$");
        this.drawSparkline("spark-downloads", [Math.round(stats.dl * 0.75), Math.round(stats.dl * 0.88), stats.dl], "#22D3EE");
    ...
    async markAllNotificationsRead() {
        try {
            await window.edmApi.markAllNotificationsRead();
    ...
    async renderFullSystemHealth() {
        ...
        try {
            ...
            if (listEl) {
                ...
                if (keys.length === 0) {
                    listEl.innerHTML = `<div style="padding: 20px; text-align: center; color: var(--color-text-muted);">Probing microservice clusters...</div>`;
                } else {
                    listEl.innerHTML = `
                        <div style="display: flex; flex-direction: column; gap: 8px; margin-top: 12px;">
                            ${keys.map(k => {
                                const c = comps[k];
                                const isOk = c.status === 0;
                                return `
                                    <div style="display: flex; align-items: center; justify-content: space-between; padding: 10px 14px; background: var(--color-bg-subtle); border-radius: var(--radius-md); border: 1px solid var(--color-border);">
                                        <div style="display: flex; align-items: center; gap: 10px;">
                                            <i data-lucide="${isOk ? 'check-circle-2' : 'alert-triangle'}" style="width: 16px; height: 16px; color: ${isOk ? '#10B981' : '#F59E0B'};"></i>
                                            <strong style="font-size: 13px; color: var(--color-text-main);">${k}</strong>
                                        </div>
                                        <div style="display: flex; align-items: center; gap: 12px;">
                                            <span style="font-family: monospace; font-size: 11.5px; color: var(--color-text-muted);">${c.latencyMs}ms</span>
                                            <span class="badge ${isOk ? 'badge-success' : 'badge-warning'}">${isOk ? 'Operational' : 'Degraded'}</span>
                                        </div>
                                    </div>
                                `;
                            }).join("")}
            ...
            if (listEl) listEl.innerHTML = `<div style="padding: 16px; color: var(--color-danger);">Health probe error: ${e.message}</div>`;
        }
    }

    async renderApiStatus() {
        const container = document.getElementById("api-status-benchmarks-list") || document.querySelector("#view-api-status .card div[style*='flex-direction: column']");
        if (!container) return;
        ...
        try {
            ...
            const endpoints = [
                { path: "GET /api/v1/admin/dashboard/overview", latency: latency, status: "Operational" },
                { path: "GET /api/v1/admin/downloads/metrics", latency: latency + 4, status: "Operational" },
                { path: "GET /api/v1/admin/downloads/stream", latency: 12, status: "Operational" },
                { path: "POST /api/v1/client/license/validate", latency: 34, status: "Operational" },
                { path: "GET /api/v1/releases/check-update", latency: 22, status: "Operational" },
                { path: "POST /api/v1/auth/login", latency: 45, status: "Operational" },
                { path: "GET /api/v1/admin/system/health", latency: latency, status: health.isHealthy ? "Operational" : "Degraded" }
            ];
            ...
            container.innerHTML = endpoints.map(ep => `
                <div style="display: flex; justify-content: space-between; align-items: center; font-size: 13px; padding: 8px 0; border-bottom: 1px solid var(--color-border);">
                    <code style="font-family: monospace; font-size: 12.5px; color: var(--color-primary-light);">${ep.path}</code>
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <span style="font-family: monospace; font-size: 11.5px; color: var(--color-text-muted);">${ep.latency}ms</span>
                        <span class="badge ${ep.status === 'Operational' ? 'badge-success' : 'badge-danger'}">● ${ep.status}</span>
                    </div>
                </div>
            `).join("");
        } catch (e) {
            container.innerHTML = `<div style="padding: 12px; color: var(--color-danger); font-size: 12px;">Probe failed: ${e.message}</div>`;
        }
    }
