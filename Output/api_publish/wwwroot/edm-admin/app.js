    ...
    setupEventListeners() {
        ...
        window.addEventListener("keydown", (e) => {
            ...
            if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === "e") {
                e.preventDefault();
                // Context-aware export: dashboard → full KPI report, live-map → regional CSV
                if (this.activePage === "dashboard") {
    ...
    async renderTicketsTable() {
        ...
        try {
            ...
            tbody.innerHTML = tickets.map(t => `
                <tr>
                    <td><code>${this.escapeHtml(t.ticketNumber)}</code></td>
                    <td><strong>${this.escapeHtml(t.customerName || t.customerEmail)}</strong></td>
                    <td><span class="badge ${t.priority === 'Critical' ? 'badge-danger' : (t.priority === 'High' ? 'badge-warning' : 'badge-neutral')}">${this.escapeHtml(t.priority)}</span></td>
                    <td>${this.escapeHtml(t.subject)}</td>
                    <td><span class="badge ${t.status === 'Resolved' ? 'badge-success' : (t.status === 'Open' ? 'badge-primary' : 'badge-warning')}">${this.escapeHtml(t.status)}</span></td>
                    <td style="font-size: 11.5px; color: var(--color-text-muted);">${new Date(t.createdAtUtc).toLocaleDateString()}</td>
                    <td style="text-align: right;">
                        <button class="btn btn-secondary btn-sm" onclick="window.edmApp.openTicketThreadModal('${this.escapeHtml(t.id)}')">
    ...
    async openTicketThreadModal(ticketId) {
        ...
        try {
            ...
            if (metaBox) {
                metaBox.innerHTML = `
                    <div>
                        <strong style="font-size: 14px; color: var(--color-text-main);">${this.escapeHtml(ticket.ticketNumber)} — ${this.escapeHtml(ticket.subject)}</strong>
                        <p style="font-size: 11.5px; color: var(--color-text-muted);">${this.escapeHtml(ticket.customerName)} (${this.escapeHtml(ticket.customerEmail)}) • Priority: <strong>${this.escapeHtml(ticket.priority)}</strong></p>
                    </div>
                    <span class="badge badge-primary">${this.escapeHtml(ticket.status)}</span>
                `;
            }
                ...
                messagesList.innerHTML = (ticket.messages || []).map(m => `
                    <div style="padding: 10px 12px; border-radius: var(--radius-md); background: ${m.senderType === 'Staff' ? 'rgba(99, 102, 241, 0.08)' : 'var(--color-bg-subtle)'}; border: 1px solid var(--color-border);">
                        <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                            <strong style="font-size: 12px; color: var(--color-text-main);">${this.escapeHtml(m.senderName)} (${this.escapeHtml(m.senderType)})</strong>
                            <span style="font-size: 10.5px; color: var(--color-text-muted);">${new Date(m.createdAtUtc).toLocaleString()}</span>
                        </div>
                        <p style="font-size: 12.5px; color: var(--color-text-secondary); line-height: 1.4;">${this.escapeHtml(m.messageContent)}</p>
                    </div>
                `).join("");
            }
        } catch (e) {
