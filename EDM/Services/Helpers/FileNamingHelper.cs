    ...
    public static class FileNamingHelper
    {
        ...
        public static string SanitizeFileName(string name)
        {
            try
            {
                ...
                if (name.Contains("..") || Path.IsPathRooted(name) || name.Contains('/') || name.Contains('\\'))
                {
                ...
                // Guard Windows DOS reserved device names (CON, PRN, AUX, NUL, COM1-9, LPT1-9)
                string baseWithoutExt = Path.GetFileNameWithoutExtension(name);
                string ext = Path.GetExtension(name);

                if (ReservedWindowsDeviceNames.Contains(baseWithoutExt))
                {
                    name = $"{baseWithoutExt}_file{ext}";
                }
