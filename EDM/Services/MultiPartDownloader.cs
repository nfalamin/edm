...
{
    public record ChunkProgressInfo(
        int Index, 
        long Downloaded, 
        long TotalBytes, 
        bool IsActive, 
    ...
    public class MultiPartDownloader : IDisposable
    {
                ...
                while (!scheduler.IsFullyCompleted() && !fallbackCts.IsCancellationRequested)
                {
                    try
                    {
                        ...
                        foreach (var seg in segments)
                        {
                            ...
                            string segState = seg.State == SegmentState.Downloading ? "Downloading" : (seg.State == SegmentState.Completed ? "Completed" : "Queued");
                            long segRemaining = Math.Max(0, seg.TotalBytes - seg.BytesDownloaded);
                            chunkStatsMap[seg.Id] = new ChunkProgressInfo(seg.Id, seg.BytesDownloaded, seg.TotalBytes, isActive, segSpeed, segState, seg.Start, seg.End, segRemaining);
                        }
        ...
        internal async Task<EDM.Models.VerificationResult> MergeFilesAsync(IEnumerable<string> chunkFiles, string destinationFilePath, CancellationToken cancellationToken, DurableDownloadState? metaState = null, string? expectedHash = null)
        {
            ...
            byte[] mergeBuffer = System.Buffers.ArrayPool<byte>.Shared.Rent(1024 * 1024); // 1MB merge buffer
            try
            {
                using (var destStream = new FileStream(tempMergePath, FileMode.Create, FileAccess.Write, FileShare.None, 1024 * 1024, FileOptions.Asynchronous | FileOptions.SequentialScan))
                {
