...
namespace EDM.Services
{
    ...
    public class DownloadOrchestrator
    {
            ...
            Action<string>? diagnosticLogger = null)
        {
            ...
            // Check if URL is a YouTube / video streaming page URL
            bool isWebStreaming = DownloadService.IsVideoStreamingUrl(item.Url) ||
                                  (item.VideoUrl != null && DownloadService.IsVideoStreamingUrl(item.VideoUrl)) ||
                                  (item.Url != null && (item.Url.Contains("youtube.com") || item.Url.Contains("youtu.be")));

            bool hasDirectMediaStreams = !string.IsNullOrWhiteSpace(item.VideoUrl) &&
                                         !string.IsNullOrWhiteSpace(item.AudioUrl) &&
                                         !item.VideoUrl.Contains("youtube.com/watch") &&
                                         !item.VideoUrl.Contains("youtu.be/") &&
                                         (item.VideoUrl.Contains("googlevideo.com") || item.VideoUrl.EndsWith(".mp4") || item.VideoUrl.EndsWith(".webm") || item.VideoUrl.EndsWith(".ts"));

            if (item.RequiresFfmpegMerge && hasDirectMediaStreams)
            {
                try
                {
                    LoggingService.Log($"[DownloadOrchestrator] Routing adaptive direct media streams for '{item.FileName}' ({item.Quality}) to MediaMergeService.");
                ...
                {
                    LoggingService.Log($"[DownloadOrchestrator] Direct stream merge failed ({ex.Message}). Falling back to robust yt-dlp media engine...");
                }
            }

            if (isWebStreaming && _ytDlp != null)
            {
                LoggingService.Log($"[DownloadOrchestrator] Routing streaming URL '{item.Url}' to YtDlpService with format '{item.FormatArg ?? item.Quality}'.");
                string streamingTargetUrl = !string.IsNullOrWhiteSpace(item.Url) ? item.Url : item.VideoUrl!;
            ...
            string effectiveUrl = (!string.IsNullOrWhiteSpace(item.VideoUrl) ? item.VideoUrl : item.Url) ?? string.Empty;
            ...
            Action<string>? diagnosticLogger = null)
        {
            ...
            try
            {
                ...
                {
                    LoggingService.Log($"[DownloadOrchestrator] SFTP Protocol URL detected. Routing to SftpDownloadService: {sanitizedUrl}");
                    info.Status = "Connecting to SFTP Server...";
                    info.ServerSupportsResume = true;
                    progressReporter.Report(info);

                    var sftpService = new SftpDownloadService();
                    System.Net.NetworkCredential? netCred = credentials != null 
                        ? credentials.ToNetworkCredential() 
                        : null;

                    if (netCred == null && ProtocolDetector.TryExtractCredentials(url, out string cleanSftpUrl, out var extractedCred))
                    {
                        netCred = extractedCred;
                        url = cleanSftpUrl;
                    }

                    await sftpService.DownloadSftpAsync(url, savePath, progressReporter, pauseToken, combinedSpeedProvider, cancellationToken, netCred).ConfigureAwait(false);
                    return;
                }
                ...
                if (probeResult.TotalBytes.HasValue && probeResult.TotalBytes.Value > 0 && probeResult.ServerSupportsResume)
                {
                    ...
                    else
                    {
                        ...
                        {
                            segmentsToUse = 32; // fallback: 32 segments for maximum speed
                        }
                    }
            ...
            catch (Exception ex)
            {
                ...
                try { SoundNotificationService.Instance.PlayEvent(SoundEvent.DownloadFailed); } catch { }
                progressReporter.Report(info);
                throw;
            }
            finally
        ...
        private async Task TriggerPostDownloadNextGenPipelineAsync(string savePath, string url, double measuredSpeedBytesPerSec = 0)
        {
            try
            {
                ...
                try
                {
                    SoundNotificationService.Instance.PlayEvent(SoundEvent.DownloadCompleted);
                }
                catch { }
            }
            catch (Exception ex)
