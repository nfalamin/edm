...
namespace EDM.Services
{
    public class MediaVariantOption
    {
        ...
        public string AudioTrackName { get; set; } = string.Empty;
        public string SourceProtocol { get; set; } = "HTTP";
        ...
        public string FormattedDetails
        {
            get
            {
                if (IsAudioOnly)
                {
                    ...
                    string lang = !string.IsNullOrEmpty(AudioLanguage) ? $" • {AudioLanguage}" : "";
                    return $"{Container.ToUpperInvariant()} Audio • {audioInfo}{lang} • {FormattedSize}";
                }
                ...
                if (!string.IsNullOrWhiteSpace(AudioLanguage)) parts.Add($"Audio: {AudioLanguage}");
    ...
    public class MediaVariantResolver
    {
        ...
        public static async Task<string?> FetchYouTubeTitleFastAsync(string mediaUrl, CancellationToken ct)
        {
            try
            {
        ...
        private async Task<MediaVariantResult> ResolveHlsVariantsAsync(Uri uri, string? cookies, CancellationToken ct)
        {
            ...
            if (playlist.IsMaster && playlist.Variants.Any())
            {
                ...
                // Add Audio Only options for all distinct audio tracks
                foreach (var audioTrack in playlist.AudioTracks.Where(a => !string.IsNullOrEmpty(a.Uri)))
                {
                    string label = !string.IsNullOrEmpty(audioTrack.Name) ? $"Audio Only ({audioTrack.Name})" : "Audio Only (AAC)";
                    result.Variants.Add(new MediaVariantOption
                    {
                        QualityLabel = label,
            ...
            result.Variants = SortVariants(result.Variants);
            return result;
        }
        ...
        private async Task<MediaVariantResult> ResolveDashVariantsAsync(Uri uri, string? cookies, CancellationToken ct)
        {
            ...
            if (manifest.VideoRepresentations.Any())
            {
                ...
                {
                    long estSize = -1;
                    if (manifest.TotalDurationSeconds > 0 && rep.Bandwidth > 0)
                    {
                        estSize = (long)(manifest.TotalDurationSeconds * (rep.Bandwidth / 8.0));
                    }
                    ...
                    result.Variants.Add(new MediaVariantOption
                    {
                        ...
                        AudioLanguage = primaryAudio?.Language ?? string.Empty,
                        EstimatedSizeBytes = estSize,
                        SourceProtocol = "DASH"
                    });
                }
            ...
            foreach (var audioRep in manifest.AudioRepresentations)
            {
                string label = !string.IsNullOrEmpty(audioRep.Language)
                    ? $"Audio Only ({audioRep.Language})"
                    : (audioRep.Bandwidth > 0 ? $"Audio Only ({audioRep.Bandwidth / 1000} kbps)" : "Audio Only (AAC)");
                ...
                result.Variants.Add(new MediaVariantOption
                {
                    ...
                    HasAudio = true,
                    Container = "m4a",
                    Bitrate = audioRep.Bandwidth,
                    AudioLanguage = audioRep.Language,
                    AudioChannels = audioRep.AudioChannels,
                    AudioSamplingRate = audioRep.AudioSamplingRate,
                    Codec = string.IsNullOrEmpty(audioRep.Codecs) ? "AAC" : audioRep.Codecs,
                    SourceProtocol = "DASH"
                });
            }
        ...
        public static List<MediaVariantOption> SortVariants(IEnumerable<MediaVariantOption> variants)
        {
            if (variants == null) return new List<MediaVariantOption>();

            return variants
                .OrderBy(v => v.IsAudioOnly ? 1 : 0)
        ...
        public static MediaVariantOption? SelectBestVariant(IEnumerable<MediaVariantOption> variants, string? maxQualityPreference = null)
        {
            if (variants == null) return null;
            var list = SortVariants(variants);
            if (!list.Any()) return null;

            if (string.IsNullOrWhiteSpace(maxQualityPreference) ||
                maxQualityPreference.Equals("Best", StringComparison.OrdinalIgnoreCase) ||
                maxQualityPreference.Equals("Auto", StringComparison.OrdinalIgnoreCase))
            {
                return list.FirstOrDefault(v => !v.IsAudioOnly) ?? list.FirstOrDefault();
            }

            if (maxQualityPreference.Equals("Audio Only", StringComparison.OrdinalIgnoreCase) ||
                maxQualityPreference.Equals("Audio", StringComparison.OrdinalIgnoreCase))
            {
                return list.FirstOrDefault(v => v.IsAudioOnly) ?? list.LastOrDefault();
            }

            int targetHeight = 1080;
            var match = Regex.Match(maxQualityPreference, @"(\d+)");
            if (match.Success && int.TryParse(match.Groups[1].Value, out int parsed))
            {
                targetHeight = parsed;
            }

            // Find best video variant satisfying Height <= targetHeight
            var underCap = list.Where(v => !v.IsAudioOnly && v.Height > 0 && v.Height <= targetHeight)
                               .OrderByDescending(v => v.Height)
                               .ThenByDescending(v => v.Bitrate)
                               .FirstOrDefault();

            if (underCap != null) return underCap;

            // Fallback to lowest video above cap, or best available
            return list.Where(v => !v.IsAudioOnly).OrderBy(v => v.Height).FirstOrDefault() ?? list.FirstOrDefault();
        }
