...
namespace EDM.Services
{
    ...
    public static class BrowserExtensionInstaller
    {
        ...
        public static string ResolveNativeHostExecutable()
        {
            ...
            // Priority 1: EDM.NativeHost.exe next to EDM.exe (same folder, production layout)
            ...
            // Priority 2: EDM.NativeHost.exe in Release sibling folder
            string[] searchRoots = new[]
            {
                Path.GetFullPath(Path.Combine(appDir, @"..\EDM.NativeHost\bin\Release\net10.0-windows")),
                Path.GetFullPath(Path.Combine(appDir, @"..\..\EDM.NativeHost\bin\Release\net10.0-windows")),
                Path.GetFullPath(Path.Combine(appDir, @"..\..\..\EDM.NativeHost\bin\Release\net10.0-windows")),
                Path.GetFullPath(Path.Combine(appDir, @"..\..\..\..\EDM.NativeHost\bin\Release\net10.0-windows")),
                Path.GetFullPath(Path.Combine(appDir, @"..\EDM.NativeHost\bin\Debug\net10.0-windows")),
                Path.GetFullPath(Path.Combine(appDir, @"..\..\EDM.NativeHost\bin\Debug\net10.0-windows")),
                Path.GetFullPath(Path.Combine(appDir, @"..\..\..\EDM.NativeHost\bin\Debug\net10.0-windows")),
                Path.GetFullPath(Path.Combine(appDir, @"..\..\..\..\EDM.NativeHost\bin\Debug\net10.0-windows")),
            };
            ...
            {
                var candidate = Path.Combine(root, "EDM.NativeHost.exe");
                if (File.Exists(candidate))
                {
                    LoggingService.Log($"[BrowserExtensionInstaller] Found NativeHost at: {candidate}");
                    return Path.GetFullPath(candidate);
                }
            }

            // Priority 3: Use EDM.exe itself as fallback native host (it handles --native-host mode)
            string edmExe = Path.Combine(appDir, "EDM.exe");
            if (File.Exists(edmExe)) return Path.GetFullPath(edmExe);

            // Default expectation path (will be created after publish)
        ...
        public static string GenerateChromiumManifestJson(string exePath)
        {
            ...
            var manifestObj = new
            {
                ...
                allowed_origins = new[]
                {
                    ...
                    // Additional common downloader extension IDs
                    "chrome-extension://pojhpobkodgamjkjnfbjfegaglhfgjkn/",
                    "chrome-extension://gaklfbfbepehonmadlfggpnhbkaejick/"
                }
            };
