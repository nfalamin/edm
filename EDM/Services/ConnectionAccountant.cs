...
namespace EDM.Services
{
    ...
    public sealed class ConnectionAccountant
    {
        ...
        /// <summary>
        /// Identifies underperforming workers whose throughput is below the 10th percentile or threshold ratio of cluster throughput.
        /// </summary>
        public List<string> DetectSlowWorkers(double thresholdRatio = 0.25, double percentile = 0.10)
        {
            var activeWorkers = _workers.Values
                .Where(w => (w.State == WorkerState.Downloading || w.State == WorkerState.Writing) && w.CurrentThroughputBps > 0)
                .ToList();
            ...
            int pIndex = Math.Max(0, (int)(speeds.Count * percentile));
            double pSpeed = speeds[pIndex];
            double slowThreshold = Math.Min(pSpeed, medianSpeed * thresholdRatio);

            return activeWorkers
                .Where(w => w.CurrentThroughputBps < slowThreshold && (DateTime.UtcNow - w.StartTime).TotalSeconds > 2.5)
                .Select(w => w.WorkerId)
                .ToList();
        }
