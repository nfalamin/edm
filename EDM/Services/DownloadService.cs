...
namespace EDM.Services
{
    public class DownloadService : IDownloadService
    {
        ...
        private static int GetMaxConnectionsPerServer()
        {
            ...
            catch (Exception) { /* Reflection best-effort, return default */ return -1; }
        }
            ...
            DownloadCredentials? credentials = null)
        {
            ...
            try
            {
                ...
                using (var response = await httpClient.SendAsync(singleReq, HttpCompletionOption.ResponseHeadersRead, cancellationToken))
                {
                    ...
                    using (var fileStream = new FileStream(tempDest, FileMode.Create, FileAccess.Write, FileShare.None, 8 * 1024 * 1024, true))
                    {
                    byte[]? buffer = null;
                    try
                    {
                        buffer = System.Buffers.ArrayPool<byte>.Shared.Rent(8 * 1024 * 1024); // 8MB — was 128KB
                        int bytesRead;
                        ...
                        while ((bytesRead = await contentStream.ReadAsync(buffer, 0, buffer.Length, cancellationToken)) > 0)
                        {
                            ...
                            if (elapsed.TotalMilliseconds >= 200) // was 500ms — more responsive UI
                            {
