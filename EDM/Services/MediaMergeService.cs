...
namespace EDM.Services
{
    ...
    public sealed class MediaMergeService
    {
        ...
        /// <summary>
        /// Extracts or remuxes an audio stream from an input media file into the specified output audio container using FFmpeg.
        /// </summary>
            ...
            bool convertToMp3 = false)
        {
            ...
            if (string.IsNullOrWhiteSpace(outputPath)) throw new ArgumentException("outputPath is required", nameof(outputPath));
            if (!File.Exists(inputPath)) throw new FileNotFoundException("Input file for audio extraction was not found.", inputPath);
            ...
            if (string.IsNullOrEmpty(exe))
            {
                throw new FileNotFoundException("FFmpeg is required for audio extraction/remuxing, but was not found on your system.");
            }
            ...
            psi.ArgumentList.Add("-vn"); // Disable video stream

            if (convertToMp3 || outputPath.EndsWith(".mp3", StringComparison.OrdinalIgnoreCase))
            {
            ...
            {
                psi.ArgumentList.Add("-c:a");
                psi.ArgumentList.Add("copy"); // Lossless remuxing
            }
            ...
            if (proc != null)
            {
                ...
                {
                    string err = await proc.StandardError.ReadToEndAsync().ConfigureAwait(false);
                    LoggingService.Log($"[MediaMergeService.ExtractAudioAsync] FFmpeg exit code {proc.ExitCode}: {err}");
                    throw new InvalidOperationException($"FFmpeg audio extraction failed: {err}");
                }
            }
            else
            {
                throw new InvalidOperationException("Failed to launch FFmpeg process for audio extraction.");
            }
        }
