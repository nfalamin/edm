    ...
    public class AdaptiveConnectionController
    {
        ...
        public int EvaluateConnectionCount(long totalFileSize, bool isMeteredNetwork, int? userLimit = null)
        {
            lock (_lock)
            {
                ...
                if (totalFileSize > 0 && totalFileSize < 10 * 1024 * 1024)
                {
                    effectiveMax = Math.Min(effectiveMax, 8);
                }
                else if (totalFileSize > 0 && totalFileSize < 50 * 1024 * 1024)
                {
                    effectiveMax = Math.Min(effectiveMax, 16);
                }
                // Files >= 50 MB: full 32-thread budget, no cap
