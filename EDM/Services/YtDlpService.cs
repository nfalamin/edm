...
namespace EDM.Services
{
    public static class YtDlpOutputParser
    {
        ...
        public static bool TryParseProgress(string? line, out double percent, out long totalBytes, out long bytesReceived, out double speedBps, out double etaSec)
        {
            ...
            if (m.Success)
            {
                ...
                // Fallback ETA calculation if yt-dlp didn't provide it
                if (etaSec <= 0 && speedBps > 0 && totalBytes > bytesReceived)
                {
                    etaSec = (double)(totalBytes - bytesReceived) / speedBps;
                }
    ...
    public class YtDlpService
    {
        ...
        private async Task DownloadInternalAsync(string url, string outputPath, string formatArg, Action<int, string>? progress, CancellationToken ct, string? cookies, bool isRetry)
        {
            ...
            psi.ArgumentList.Add("32"); // 32 concurrent fragments for maximum speed
            psi.ArgumentList.Add("--buffer-size");
            psi.ArgumentList.Add("32M");  // doubled for high-speed connections
