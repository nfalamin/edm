...
namespace EDM.Services
{
    ...
    public class ControlPlaneClient
    {
        ...
        public async Task<bool> LoginWithFirebaseAsync(string firebaseIdToken, CancellationToken ct = default)
        {
            ...
            try
            {
                string endpoint = $"{GetApiBaseUrl()}/api/v1/auth/firebase";
                var payload = new
                {
            ...
            {
                LoggingService.Log($"[ControlPlaneClient] Firebase Login failed: {ex.Message}");
                SetSecurityState(AccountSecurityState.Offline);
            }
            finally
            {
