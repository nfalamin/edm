...
namespace EDM.ViewModels
{
    ...
    public partial class DownloadManagerViewModel : ViewModelBase
    {
        ...
        public async Task LoadHistoryFromDatabaseAsync()
        {
            try
            {
                ...
                await System.Windows.Application.Current.Dispatcher.InvokeAsync(() =>
                {
                    ...
                    // Step 19.4 Crash Recovery: Heal stale 'Downloading' jobs to 'Paused'
                    DownloadQueueScheduler.Instance.RecoverStaleDownloads(AllDownloads);
        ...
        [RelayCommand]
        public void StartQueue(string? queueId = null)
        {
            string qId = queueId ?? "default";
            DownloadQueueScheduler.Instance.StartQueue(qId);

            var nextQueued = DownloadQueueScheduler.Instance.TryGetNextDownloadToStart(qId);
            if (nextQueued != null)
            {
                var item = AllDownloads.FirstOrDefault(d => d.Id.ToString("N") == nextQueued.DownloadId);
                if (item != null && (item.Status == "Queued" || item.Status == "Waiting" || item.Status == "Paused"))
                {
                    _ = StartDownloadProcessAsync(item);
                }
            }
        }

        [RelayCommand]
        public void StopQueue(string? queueId = null)
        {
            string qId = queueId ?? "default";
            DownloadQueueScheduler.Instance.StopQueue(qId);
        }

        [RelayCommand]
        public void PauseQueue(string? queueId = null)
        {
            string qId = queueId ?? "default";
            DownloadQueueScheduler.Instance.PauseQueue(qId);

            foreach (var d in AllDownloads.Where(x => string.Equals(x.QueueId, qId, StringComparison.OrdinalIgnoreCase) && x.Status.Contains("Downloading")).ToList())
            {
                d.PauseSource.Pause();
                d.Status = "Paused";
                d.TransferRate = "0 B/s";
            }
            ApplyFilter();
            RecalculateMetrics();
        }
        ...
        [RelayCommand]
        public void ResumeQueue(string? queueId = null)
        {
            string qId = queueId ?? "default";
            DownloadQueueScheduler.Instance.ResumeQueue(qId);
        ...
        public void MoveUp(DownloadItem? item)
        {
            ...
            if (DownloadQueueScheduler.Instance.MoveUp(idStr))
            {
                ApplyFilter();
            }
        }
        ...
        public void MoveDown(DownloadItem? item)
        {
            if (item == null) return;
            string idStr = item.Id.ToString("N");
            if (DownloadQueueScheduler.Instance.MoveDown(idStr))
            {
                ApplyFilter();
            }
        }
        ...
        /// <summary>
        /// Manually starts a scheduled or waiting download immediately, overriding closed schedule windows.
        /// </summary>
        [RelayCommand]
        public void StartNow(DownloadItem? item)
        {
            if (item == null) return;
            SchedulerService.Instance.SetManualOverride(item.Id.ToString("N"), allowImmediateStart: true);
            _ = StartDownloadProcessAsync(item);
        }
