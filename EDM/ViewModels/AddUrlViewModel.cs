        ...
        public void StartDownload()
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(ResourceTitle))
                {
                    ...
                    {
                        string? fastTitle = null;
                        try { fastTitle = MediaVariantResolver.FetchYouTubeTitleFastAsync(normalizedUrl, CancellationToken.None).GetAwaiter().GetResult(); } catch { }
                        fileName = !string.IsNullOrWhiteSpace(fastTitle) ? $"{FileNamingHelper.SanitizeFileName(fastTitle)}.mp4" : $"YouTube_Video_{DateTime.Now:yyyyMMdd_HHmmss}.mp4";
                    }
                    else
                    {
