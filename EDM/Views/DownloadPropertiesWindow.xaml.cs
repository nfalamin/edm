...
namespace EDM.Views
{
    ...
    public partial class DownloadPropertiesWindow : Window
    {
        ...
        private void PopulateProperties()
        {
            ...
            TxtDate.Text = !string.IsNullOrWhiteSpace(_item.LastTryDate) ? _item.LastTryDate : DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            TxtDescription.Text = !string.IsNullOrWhiteSpace(_item.Description) ? _item.Description : (!string.IsNullOrWhiteSpace(_item.VerificationMessage) ? _item.VerificationMessage : "Normal");

            TxtReferer.Text = !string.IsNullOrWhiteSpace(_item.Referer) ? _item.Referer : "--";
            TxtUserAgent.Text = !string.IsNullOrWhiteSpace(_item.UserAgent) ? _item.UserAgent : "EDM/1.0 (Windows NT 10.0; Win64; x64)";
            
            string mediaInfo = string.Empty;
            if (!string.IsNullOrWhiteSpace(_item.Quality)) mediaInfo += $"Quality: {_item.Quality} ";
            if (!string.IsNullOrWhiteSpace(_item.Container)) mediaInfo += $"Container: {_item.Container} ";
            if (!string.IsNullOrWhiteSpace(_item.Codec)) mediaInfo += $"Codec: {_item.Codec} ";
            TxtMediaInfo.Text = !string.IsNullOrWhiteSpace(mediaInfo) ? mediaInfo.Trim() : (!string.IsNullOrWhiteSpace(_item.DesiredFormat) ? _item.DesiredFormat : "--");
