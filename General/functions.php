...
function edm_rest_handle_feedback_submission( WP_REST_Request $request ) {
    ...
    $version  = isset( $body['version'] ) ? sanitize_text_field( $body['version'] ) : 'v2.1.0';
    $details  = isset( $body['details'] ) ? sanitize_textarea_field( $body['details'] ) : '';

    if ( empty( $name ) || empty( $email ) || empty( $subject ) || empty( $details ) ) {
        return new WP_REST_Response( [
            'success' => false,
            'message' => 'Please provide name, email, subject, and detailed message.',
        ], 400 );
    }
...
function edm_rest_handle_contact_submission( WP_REST_Request $request ) {
    ...
    $service_type = isset( $body['service_type'] ) ? sanitize_text_field( $body['service_type'] ) : 'General Inquiry';
    $budget       = isset( $body['budget'] ) ? sanitize_text_field( $body['budget'] ) : 'Flexible';
