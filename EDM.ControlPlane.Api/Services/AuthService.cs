...
namespace EDM.ControlPlane.Api.Services
{
    ...
    public record UserDto(Guid Id, string Username, string Email, string Role, bool TwoFactorEnabled = false, string? FirebaseUid = null, string? DisplayName = null, string? PhotoUrl = null);

    public record UserPasskeyDto(Guid Id, string DeviceName, DateTime CreatedAtUtc, DateTime? LastUsedAtUtc);
    ...
    public interface IAuthService
    {
        ...
        Task<AuthResult> VerifyFirebaseLoginAsync(string firebaseIdToken, Guid? installationId, string? userAgent = null, string? rawIp = null);
        ...
        Task<List<UserPasskeyDto>> GetUserPasskeysAsync(Guid userId);
        Task<AuthResult> DeletePasskeyAsync(Guid userId, Guid passkeyId);
        Task<AuthResult> RenamePasskeyAsync(Guid userId, Guid passkeyId, string newDeviceName);
    }
    ...
    public class AuthService : IAuthService
    {
            ...
            IFirebaseAuthService? firebaseAuthService = null)
        {
            ...
            _firebaseAuthService = firebaseAuthService ?? new FirebaseAuthService(new Microsoft.Extensions.Configuration.ConfigurationBuilder().Build());
        }
        ...
        public async Task<AuthResult> VerifyFirebaseLoginAsync(string firebaseIdToken, Guid? installationId, string? userAgent = null, string? rawIp = null)
        {
            if (string.IsNullOrWhiteSpace(firebaseIdToken))
            {
                return new AuthResult(false, "Firebase ID token is required.", ErrorCode: "INVALID_TOKEN");
            }

            var firebasePayload = await _firebaseAuthService.ValidateFirebaseTokenAsync(firebaseIdToken);
            if (firebasePayload == null)
            {
                return new AuthResult(false, "Invalid or expired Firebase ID token.", ErrorCode: "INVALID_FIREBASE_TOKEN");
            }

            string email = firebasePayload.Email.ToLowerInvariant().Trim();
            var user = await _dbContext.Users.FirstOrDefaultAsync(u => 
                (!string.IsNullOrEmpty(u.FirebaseUid) && u.FirebaseUid == firebasePayload.Uid) || 
                u.Email == email);

            if (user == null)
            {
                // Auto-provision user account with Firebase identity
                string usernameBase = !string.IsNullOrWhiteSpace(firebasePayload.DisplayName)
                    ? System.Text.RegularExpressions.Regex.Replace(firebasePayload.DisplayName, @"[^a-zA-Z0-9_\-\.]", "_")
                    : email.Split('@')[0];
                ...
                user = new User
                {
                    ...
                    Username = username,
                    Email = email,
                    PasswordHash = string.Empty, // Managed via Firebase Auth
                    FirebaseUid = firebasePayload.Uid,
                    DisplayName = firebasePayload.DisplayName,
                    PhotoUrl = firebasePayload.PhotoUrl,
                    Role = UserRole.USER,
                    IsActive = true,
                    IsEmailVerified = firebasePayload.EmailVerified,
                    CreatedAtUtc = DateTime.UtcNow,
                    UpdatedAtUtc = DateTime.UtcNow
                };
                ...
                await _auditLogger.LogActionAsync(
                    ...
                    targetEntity: "User",
                    targetId: user.Id.ToString(),
                    detailsJson: $"{{\"firebaseUid\":\"{firebasePayload.Uid}\",\"provider\":\"{firebasePayload.ProviderId}\"}}",
                    correlationId: Guid.NewGuid().ToString("N"),
                    resultStatus: "SUCCESS",
                    rawIpAddress: rawIp);
            }
            else
            {
                ...
                {
                    user.FirebaseUid = firebasePayload.Uid;
                    modified = true;
                }
                if (!string.IsNullOrEmpty(firebasePayload.DisplayName) && user.DisplayName != firebasePayload.DisplayName)
                {
                    user.DisplayName = firebasePayload.DisplayName;
                    modified = true;
                }
                if (!string.IsNullOrEmpty(firebasePayload.PhotoUrl) && user.PhotoUrl != firebasePayload.PhotoUrl)
                {
                    user.PhotoUrl = firebasePayload.PhotoUrl;
                    modified = true;
                }
                if (firebasePayload.EmailVerified && !user.IsEmailVerified)
                {
        ...
        private async Task<AuthResult> IssueSessionAsync(User user, Guid installId, string? userAgent, string? rawIp, bool rememberDevice = false)
        {
            ...
            return new AuthResult(
                ...
                User: new UserDto(user.Id, user.Username, user.Email, user.Role.ToString(), user.TwoFactorEnabled, user.FirebaseUid, user.DisplayName, user.PhotoUrl));
        }
        ...
        public async Task<List<UserPasskeyDto>> GetUserPasskeysAsync(Guid userId)
        {
            return await _dbContext.UserPasskeys
                .Where(p => p.UserId == userId)
                .OrderByDescending(p => p.CreatedAtUtc)
                .Select(p => new UserPasskeyDto(p.Id, p.DeviceName, p.CreatedAtUtc, p.LastUsedAtUtc))
                .ToListAsync();
        }

        public async Task<AuthResult> DeletePasskeyAsync(Guid userId, Guid passkeyId)
        {
            var passkey = await _dbContext.UserPasskeys
            ...
            if (passkey == null)
            {
                return new AuthResult(false, "Passkey not found.", ErrorCode: "PASSKEY_NOT_FOUND");
            }
            ...
            await _auditLogger.LogActionAsync(
                ...
                targetEntity: "UserPasskey",
                targetId: passkeyId.ToString(),
                detailsJson: $"{{\"deviceName\":\"{passkey.DeviceName}\"}}",
                correlationId: Guid.NewGuid().ToString("N"),
                resultStatus: "SUCCESS",
                rawIpAddress: null);

            return new AuthResult(true, "Passkey removed successfully.");
        }

        public async Task<AuthResult> RenamePasskeyAsync(Guid userId, Guid passkeyId, string newDeviceName)
        {
            if (string.IsNullOrWhiteSpace(newDeviceName))
            {
                return new AuthResult(false, "Device name cannot be empty.", ErrorCode: "INVALID_NAME");
            }
            ...
            passkey.DeviceName = newDeviceName.Trim();
            await _dbContext.SaveChangesAsync();
            ...
            await _auditLogger.LogActionAsync(
                ...
                targetEntity: "UserPasskey",
                targetId: passkeyId.ToString(),
                detailsJson: $"{{\"newName\":\"{passkey.DeviceName}\"}}",
                correlationId: Guid.NewGuid().ToString("N"),
                resultStatus: "SUCCESS",
                rawIpAddress: null);

            return new AuthResult(true, "Passkey renamed successfully.");
        }
