...
namespace EDM.ControlPlane.Api.Controllers
{
    public record DeviceHeartbeatRequest(
        ...
        List<DriveStorageDto>? StorageDrives);

    public record DriveStorageDto(
        string DriveName,
        string VolumeLabel,
        string DriveFormat,
        long TotalSizeBytes,
    ...
    public record SyncHistoryRequest(
        Guid InstallationId,
        List<DownloadHistoryItemDto> Records);

    public record DownloadHistoryItemDto(
        string Url,
        string FileName,
        string? Category,
        long FileSizeBytes,
    ...
    public class RemoteControlController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetUserDevicesAsync()
        {
            ...
            var ownedDeviceIds = await _dbContext.Devices
        ...
        public async Task<IActionResult> DeviceHeartbeatAsync([FromBody] DeviceHeartbeatRequest request)
        {
            ...
            if (device == null)
            {
                device = new Device
                {
                    ...
                    StorageJson = (request.StorageDrives != null && request.StorageDrives.Count > 0) ? JsonSerializer.Serialize(request.StorageDrives) : null,
            ...
            else
            {
                ...
                if (!device.UserId.HasValue) device.UserId = userId.Value;
                ...
                if (request.StorageDrives != null && request.StorageDrives.Count > 0)
                {
                    device.StorageJson = JsonSerializer.Serialize(request.StorageDrives);
                }
        ...
        public async Task<IActionResult> SendRemoteCommandAsync([FromBody] CreateRemoteCommandRequest request)
        {
            ...
            bool isAuthorizedDevice = (device.UserId.HasValue && device.UserId.Value == userId.Value)
                || await _dbContext.Sessions.AnyAsync(s => s.UserId == userId.Value && s.DeviceId == device.Id)
        ...
        [HttpGet("devices/{deviceId}/storage")]
        public async Task<IActionResult> GetDeviceStorageAsync([FromRoute] Guid deviceId)
        {
            var userId = GetCurrentUserId();
            if (userId == null) return Unauthorized(new { error = "UNAUTHORIZED" });

            var device = await _dbContext.Devices.FirstOrDefaultAsync(d => d.Id == deviceId);
            if (device == null) return NotFound(new { error = "DEVICE_NOT_FOUND" });

            // Verify access
            bool isAuthorized = (device.UserId.HasValue && device.UserId.Value == userId.Value)
                || await _dbContext.Sessions.AnyAsync(s => s.UserId == userId.Value && s.DeviceId == device.Id)
                || await _dbContext.LiveDownloads.AnyAsync(l => l.UserId == userId.Value && l.DeviceId == device.Id)
                || await _dbContext.SyncedFiles.AnyAsync(f => f.OwnerId == userId.Value && f.DeviceId == device.Id);
            ...
            {
                var user = await _dbContext.Users.FindAsync(userId.Value);
                if (user?.Role != UserRole.SUPER_ADMIN) return Forbid();
            }

            var drives = new List<DriveStorageDto>();
            if (!string.IsNullOrEmpty(device.StorageJson))
            {
                try
                {
                    drives = JsonSerializer.Deserialize<List<DriveStorageDto>>(device.StorageJson, new JsonSerializerOptions { PropertyNameCaseInsensitive = true }) ?? new();
                }
                catch { }
            }
            ...
            return Ok(new
            {
                ...
                clientType = device.ClientType.ToString(),
                osVersion = device.OsVersion,
                isOnline = (now - device.LastSeenAtUtc).TotalMinutes <= 2,
        ...
        public async Task<IActionResult> SyncDownloadHistoryAsync([FromBody] SyncHistoryRequest request)
        {
            if (request == null || request.InstallationId == Guid.Empty || request.Records == null)
            {
                return BadRequest(new { error = "INVALID_PAYLOAD", message = "Valid installationId and records array required." });
            }
            ...
            {
                if (string.IsNullOrWhiteSpace(rec.Url) || string.IsNullOrWhiteSpace(rec.FileName)) continue;

                // Check duplicate by URL + CompletedAtUtc
                bool exists = await _dbContext.DownloadRecords.AnyAsync(d =>
            ...
            {
                success = true,
                message = $"Successfully synced {addedCount} history record(s).",
            ...
            [FromQuery] int limit = 50)
        {
            ...
            if (!string.IsNullOrWhiteSpace(search))
            {
                ...
                query = query.Where(d => (d.FileName != null && d.FileName.ToLower().Contains(sLower)) || (d.Url != null && d.Url.ToLower().Contains(sLower)));
            }
            ...
            var records = await query
                ...
                .Select(d => new
                {
                    ...
                    downloadedAtUtc = d.DownloadedAtUtc
                })
                .ToListAsync();
